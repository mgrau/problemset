...
local function parseSolutions(div)
  ...
  if div.classes:includes("ps-solution") then
    if show_solutions then
      if quarto.doc.is_format("latex") then
        local beginEnv = '\\begin{tcolorbox}[enhanced jigsaw, left=2mm, breakable, opacityback=0, toptitle=1mm, titlerule=0mm, colbacktitle=blue!10!white, coltitle=black, colback=white, leftrule=.75mm, colframe=blue!50!black, title={Solution}, opacitybacktitle=0.6, bottomtitle=1mm, rightrule=.15mm, bottomrule=.15mm, toprule=.15mm, arc=.35mm]'
        local endEnv = '\n\\end{tcolorbox}'
      ...
      elseif quarto.doc.is_format("html") then
        local header = pandoc.Div(pandoc.Div("Solution",{class = 'callout-title-container flex-fill'}),{class = 'callout-header d-flex align-content-center'})
